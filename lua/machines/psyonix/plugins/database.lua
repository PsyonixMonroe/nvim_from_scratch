return {
    ...
    init = function()
        ...
        vim.keymap.set({ "n", "x" }, "<Leader>ms", ":DBUIToggle<CR>", { desc = "Toggle MySQL Viewer" })
        ...
        vim.g.dbs = {
            ...
            { name = 'RLStatsAggregator-Local',   url = 'mysql://root:password@127.0.0.1:6034' },
            ...
            { name = 'RLStatsAggregator-IntTest', url = 'mysql://root:password@127.0.0.1:7034' },
            ...
            { name = 'RLMP-Main',                 url = get_remote_mysql(secret, 3460) },
            { name = 'RLMP-Progression',          url = get_remote_mysql(secret, 3462) },
            { name = 'RLMP-Match',                url = get_remote_mysql(secret, 3463) },
            { name = 'RLMP-Matchmaking',          url = get_remote_mysql(secret, 3464) },
            { name = 'RLMP-Gameserver',           url = get_remote_mysql(secret, 3466) },
            { name = 'RLMP-Skill',                url = get_remote_mysql(secret, 3467) },
            { name = 'RLMP-Challenge',            url = get_remote_mysql(secret, 3468) },
        }
    end,
}
